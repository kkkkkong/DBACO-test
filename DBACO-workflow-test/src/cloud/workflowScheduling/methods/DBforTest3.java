...
public class DBforTest3 implements Scheduler{
	...
	public DBforTest3(double theta) {
		this.theta = theta;
	}
    ...
    private void calculateCost(List<Task> tasks) {
    	for(Task task : tasks) {
    		...
    		leastCost = Math.ceil((task.getTaskSize()/VM.SPEEDS[0] + maxOutTime)/VM.INTERVAL) * VM.UNIT_COSTS[0];
	...
	private Allocation allocateTask(Task task, Solution solution, Double proSubDeadline, int taskIndex) {
		...
		for(Task procedent : unallocated) {
			unallocatedSum +=averageCosts.get(procedent);
		}
