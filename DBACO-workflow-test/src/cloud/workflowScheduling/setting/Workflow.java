...
//			System.out.println(t.getName() +"\t"+t.getpURank());
	...
	public void calcCPURank(double theta) {
		double speed = VM.SPEEDS[VM.FASTEST];
		for(int j = this.size()-1; j>=0; j--) {
			double cpURank = 0;
			Task task = this.get(j);
			for(Edge outEdge : task.getOutEdges()) {
				...
				if(theta != Double.MAX_VALUE) {
					double et = child.getTaskSize() / speed;
					double tt = outEdge.getDataSize() / VM.NETWORK_SPEED;
					double d = 1-Math.pow(theta, -et / tt);
					if(d<random())
						flag = 0;
				}
				if(task.getOutEdges().size()==1) {
					Task next = task.getOutEdges().get(0).getDestination();
					if(next.getInEdges().size() == 1) 
						flag = 0;
				}
				cpURank = Math.max(cpURank, child.getcpURank() + flag * outEdge.getDataSize() / VM.NETWORK_SPEED);
			}
			task.setcpURank(cpURank + task.getTaskSize() / speed);
		}
	}
	...
	public void calcRank(){
		for(int j= this.size()-1; j>=0; j--){
			...
			for(Edge outEdge : task.getOutEdges()){
				Task child = outEdge.getDestination();				
				rank = Math.max(rank, child.getRank() + outEdge.getDataSize() / VM.NETWORK_SPEED);
			}
			double averageECT = 0.0;
			for(int i =0;i<VM.TYPE_NO;i++) {
				averageECT += task.getTaskSize()/VM.SPEEDS[i];
			}
			averageECT /= VM.TYPE_NO;
			task.setRank(rank + averageECT);
		}
	}
